...
from prismatic.vla.constants import (ACTION_DIM, ACTION_TOKEN_BEGIN_IDX, 
                                     IGNORE_INDEX, NUM_ACTIONS_CHUNK,
                                       PROPRIO_DIM, STOP_INDEX)
...
class OpenVLAServer:
    ...
    def get_server_action(self,payload: Dict[str, Any]) -> str:
        try:
            ...
            # If the action is a list, convert it to a numpy array
            # res_data = action.tolist()
            action_dict = {"success": True,"actions": [a.tolist() for a in action]}


            if double_encode:
                return JSONResponse(json_numpy.dumps(action_dict))
            else:
                # return JSONResponse(content=action_dict)
    ...
    def health_check(self) -> JSONResponse:
        ...
        """
        return JSONResponse({"status": "ok", "message": "OpenVLA server is running."})
...
class DeployConfig:
    ...
    pretrained_checkpoint: Union[str, Path] = "/data/ckpt/openvla-oft/aloha_4_tasks/100000steps/openvla-7b+my_aloha_4_tasks+b4+lr-0.0005+lora-r32+dropout-0.0--image_aug--452+my_aloha_4_tasks+b4+lr-0.0005+lora-r32+dropout-0.0--image_aug--455"     # Pretrained checkpoint path
    ...
    unnorm_key: Union[str, Path] = "my_aloha_picking_banana_new"                # Action un-normalization key
