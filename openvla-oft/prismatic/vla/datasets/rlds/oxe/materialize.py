...
from .dataset_paths import DATASET_PATH_MAP
# ##混合数据集路径映射
# DATASET_PATH_MAP = {
...
def get_oxe_dataset_kwargs_and_weights(
    ...
    for d_name, d_weight in filtered_mixture_spec:
        try:
            print(f"Loading Dataset: `{d_name}` with weight `{d_weight}`")
            print(f"Dataset Path: `{DATASET_PATH_MAP[d_name]}`")
            per_dataset_kwargs.append(
                make_oxe_dataset_kwargs(
                    d_name,
                    # data_root_dir,
                    Path(DATASET_PATH_MAP[d_name]),
                    load_camera_views,
                    load_depth,
                    load_proprio,
                    load_language,
