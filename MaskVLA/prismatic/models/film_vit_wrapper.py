...
class FiLMedPrismaticVisionBackbone(nn.Module):
    ...
    def __init__(
        ...
        # # 添加 SIGLIP 可学习参数
        # self.logit_scale = nn.Parameter(torch.ones([]) * torch.log(torch.tensor(1/0.07)))  # 初始值 ≈ ln(1/0.07)
        # self.logit_bias = nn.Parameter(torch.zeros([]))  # 初始值为0
        
        # Wrap vision transformers
        self._wrap_vit(self.vision_backbone.featurizer)  # DINOv2
        if self.vision_backbone.use_fused_vision_backbone:
            self._wrap_vit(self.vision_backbone.fused_featurizer)  # SigLIP
    ...
    def forward(self, pixel_values: torch.Tensor, language_embeddings: torch.Tensor) -> torch.Tensor:
        ...
        # print("i am in the FiLMedPrismaticVisionBackbone forward function")
        if self.get_num_images_in_input() == 1:
            if not self.vision_backbone.use_fused_vision_backbone:#只用了 SigLIP，不融合 DINOv2
                return self.vision_backbone.featurizer(pixel_values, average_language_embedding)

            #用了双分支（SigLIP + DINOv2），pixel_values.shape == [B, C, H, W]，C=2 * 3因为双分支
            # Split `pixel_values :: [bsz, 2 * 3, resolution, resolution]` =>> featurize =>> channel stack
            img, img_fused = torch.split(pixel_values, [3, 3], dim=1)#分离后[B, 3, H, W]
            #SigLIP
            ...
            for img in images:
                ...
                # print(f"Processing image with shape: {img_regular.shape} and {img_fused.shape}")
                #Processing image with shape: torch.Size([4, 3, 224, 224]) and torch.Size([4, 3, 224, 224])
                # Get patches from both SigLIP and DINOv2 vision transformers
                patches = self.vision_backbone.featurizer(img_regular, average_language_embedding)#dino
                patches_fused = self.vision_backbone.fused_featurizer(img_fused, average_language_embedding)#siglip
                # print(f"dino patches shape: {patches.shape}, siglip patches shape: {patches_fused.shape}")
                #torch.Size([4, 256, 1024]),torch.Size([4, 256, 1152])
                # Concatenate SigLIP and DINOv2 patches along the hidden dimension
                combined_patches = torch.cat([patches, patches_fused], dim=2)
                # all_patches.append(combined_patches)
                # print(f"Combined patches shape: {combined_patches.shape}")
                # Combined patches shape: torch.Size([4, 256, 2176])
                # Collect for all images
                all_combined_patches.append(combined_patches)
                all_siglip_patches.append(patches_fused)
                all_dinov2_patches.append(patches)

            # Concatenate along the patch dimension (dim=1)
            combined = torch.cat(all_combined_patches, dim=1)     # [B, N_total, 2D]
            siglip_only = torch.cat(all_siglip_patches, dim=1)    # [B, N_total, D]
            dinov2_only = torch.cat(all_dinov2_patches, dim=1)    # [B, N_total, D]

            # # Concatenate all patches along the patch dimension
            # return torch.cat(all_patches, dim=1)
            
            # Return all three
            return combined, siglip_only, dinov2_only
