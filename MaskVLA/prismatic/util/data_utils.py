...
#数据整理器（Data Collator）​​，专门用于处理多模态数据（视觉、语言、动作）的批次填充（padding）和打包
    model_max_length: int
    pad_token_id: int
    padding_side: str = "right"
    pixel_values_dtype: torch.dtype = torch.float32
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        if "dataset_name" in instances[0]:
            ...
            dataset_names = None
        language_instructions = [instance["language_instruction"] for instance in instances] if "language_instruction" in instances[0] else None
        ...
        if language_instructions is not None:
            output["language_instructions"] = language_instructions
        return output
