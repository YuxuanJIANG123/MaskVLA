...
class Config:
    def __init__(self):
        ...
        # self.image_path = '/home/Better-oft/openvla-oft/Draw_attention_map/siglip_visual/1.png'
        self.image_path = '/home/Better-oft/openvla-oft/Draw_attention_map/My_image/pipeline-cat-chonk.jpeg'
        self.labels = ["a banana", "a cat", "a dog"]  # 要评估的标签
...
if __name__ == '__main__':
    ...
    model = model.to(config.device).eval()  # 修改：使用评估模式
    ...
    # 选择目标层 - 修改：选择注意力层而不是LayerNorm层
    # 尝试多个可能的目标层

    # # 选项1: 最后一个transformer block的注意力层
    # target_layers = [model.visual.trunk.blocks[-1].attn]
    # print("Using attention layer as target")
    
        
    # # # 选项2: 最后一个transformer block的MLP层
    # target_layers = [model.visual.trunk.blocks[-1].mlp]
    # print("Using MLP layer as target")

    # 备选: 使用原来的LayerNorm层
    target_layers = [model.visual.trunk.blocks[-1].norm1]
    print("Using LayerNorm layer as target")
    ...
    # 首先确定哪个标签概率最高
    with torch.no_grad():
        temp_features = model.encode_image(input_tensor)
        temp_text_features = model.encode_text(text)
        temp_features = F.normalize(temp_features, dim=-1)
        temp_text_features = F.normalize(temp_text_features, dim=-1)
        temp_probs = torch.sigmoid(temp_features @ temp_text_features.T * model.logit_scale.exp() + model.logit_bias)
        target_class_idx = torch.argmax(temp_probs[0]).item()
        print(f"Target class index: {target_class_idx}, label: {config.labels[target_class_idx]}")
    
    def text_target_fn(output):
        text_features = model.encode_text(text)
        text_features = F.normalize(text_features, dim=-1)
        logits = (output @ text_features.T * model.logit_scale.exp() + model.logit_bias)
        # 修改：选择概率最高的标签而不是固定第一个
    ...
    with cam_algorithm(model=model,
        ...
        # 1. 保存CAM热力图
        target_label_name = config.labels[target_class_idx].replace(" ", "_")
        cam_output_path = os.path.join(config.output_dir, f'siglip_{config.method}_cam_{target_label_name}.jpg')
        ...
        gb_output_path = os.path.join(config.output_dir, f'siglip_{config.method}_gb_{target_label_name}.jpg')
        ...
        cam_gb_output_path = os.path.join(config.output_dir, f'siglip_{config.method}_cam_gb_{target_label_name}.jpg')
