...
import sys
# sys.argv.append("--dataset_name=my_aloha_2_tasks")
...
from loss_model import VLALossModel


@dataclass
class FinetuneConfig:
    # fmt: off
    vla_path: str = "/data/ckpt/openvla/models--openvla--openvla-7b-oft/snapshots/openvla-7b"             # Path to OpenVLA model (on HuggingFace Hub or stored locally)

    # Dataset
    data_root_dir: Path = Path("/data/dataset/datasets-openvla-oft/pick_put_banana/rlds_datasets")      # Directory containing RLDS datasets
    dataset_name: str = "my_aloha_sim_hanging_mug"    # Name of fine-tuning dataset (e.g., `aloha_scoop_x_into_bowl`)
    ...
    num_steps_before_decay: int = 3000            # Number of steps before LR decays by 10x
    grad_accumulation_steps: int = 1                 # Number of gradient accumulation steps
    max_steps: int = 6000                         # Max number of training steps
    use_val_set: bool = False                        # If True, uses validation set and log validation metrics
    val_freq: int = 2000                           # (When `use_val_set==True`) Validation set logging frequency in steps
    ...
    # Logging
    wandb_entity: str = "wge2002"          # Name of WandB entity
    wandb_project: str = "aloha_debug"        # Name of WandB project
    run_id_note: Optional[str] = "parallel_dec--25_acts_chunk--continuous_acts--L1_regression--3rd_person_img--proprio_state--film"               # Extra note to add to end of run ID for logging
...
def run_forward_pass(
    ...
    # print(f"Input IDs shape: {input_ids.shape}")  # torch.Size([4, 378])
    # print(device_id)
    # print("CUDA_VISIBLE_DEVICES:", os.environ.get("CUDA_VISIBLE_DEVICES", "Not set"))

    # VLA forward pass
    with torch.autocast("cuda", dtype=torch.bfloat16):
        output: CausalLMOutputWithPast = vla( # PrismaticCausalLMOutputWithPast
            input_ids=batch["input_ids"].to(device_id),
    ...
    loss_model = VLALossModel(
        vla=vla,
        # vision_backbone=vision_backbone,
        # image_transform=image_transform,
        action_head=action_head,
        num_patches=num_patches,
        siglip_loss_weight=0.3,
        dinov2_loss_weight=0.2,
    ...
    loss, metrics = loss_model(output, batch, device_id)
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # print(f"Using device {device_id} for training")
    print("CUDA_VISIBLE_DEVICES:", os.environ.get("CUDA_VISIBLE_DEVICES", "Not set"))
    ...
    # print(type(vla.vision_backbone))#
    ...
    # print(f"Number of images in input: {cfg.num_images_in_input}")##为啥是1啊很怪
    ...
    # vla.language_model.requires_grad_(True)
    # vla.projector.requires_grad_(True)
    ...
    # print(f"Number of vision patches of images: {NUM_PATCHES}") #NUM_PATCHES=256*3
    ...
    recent_metrics = {
        ...
        # "siglip_l1_loss": deque(maxlen=cfg.grad_accumulation_steps),
        # "dinov2_l1_loss": deque(maxlen=cfg.grad_accumulation_steps),
        # "v2t_contrastive_loss": deque(maxlen=cfg.grad_accumulation_steps),
    }

    # print(f"[INFO]loading vision backbone and image transform in device {device_id}...")
    
    # my_vision_backbone, my_image_transform=load_vision_backbone_and_image_transform(device_id)
    # my_vision_backbone = wrap_ddp(my_vision_backbone, device_id, find_unused=True)

    # # my_vision_backbone.requires_grad_(False)
    # for param in my_vision_backbone.parameters():
    #     param.requires_grad = False  # 双重保险
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            loss, metrics = run_forward_pass(
                vla=vla,
                # vision_backbone=my_vision_backbone,
                # image_transform=my_image_transform,
                action_head=action_head,
