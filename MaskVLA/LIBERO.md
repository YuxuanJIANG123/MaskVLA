# MaskVLA in the LIBERO Simulation Benchmark
...
pip install -r experiments/robot/libero/libero_requirements.txt  # From MaskVLA base dir
```
...
train other methods. This step is optional since we provide pretrained MaskVLA checkpoints below.
...
The four MaskVLA checkpoints for LIBERO are available on Hugging Face:
...
* The evaluation script will run 500 trials by default (10 tasks x 50 episodes each). You can modify the number of
  ...
  other arguments in the script; we set them to the default values that work with the MaskVLA checkpoints above.
...
* The results reported in our paper were obtained using **Python 3.10.14, PyTorch 2.2.0, and our
  [custom transformers v4.40.1 fork](https://github.com/moojink/transformers-MaskVLA.git)**
...
The above training command should reproduce our MaskVLA results if `X = 8` and the 150K step checkpoint is evaluated.
