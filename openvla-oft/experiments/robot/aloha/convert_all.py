...
def concatenate_by_key(data: dict, concatenater: dict, remove_ori=True) -> dict:
    for key, value in concatenater.items():
        ...
        if not set(value).issubset(set(data.keys())):
            ...
            continue
        if len([v for v in value if v in data and isinstance(data[v], (list, np.ndarray)) and len(data[v]) > 0]) == 0:
            print(f"[ERROR] No data found to concatenate for key: {key}")
            print(f"[DEBUG] keys to concat: {value}")
            for v in value:
                print(f"  -> {v} : {type(data.get(v))}, len={len(data.get(v, [])) if isinstance(data.get(v), (list, np.ndarray)) else 'N/A'}")
            continue  # 避免后续 np.concatenate 出错
    ...
    with open(path, "rb") as f:
        ...
        def separate_data(key_data, pre_process, padding):
            ...
            for i in range(len(key_data)):
                ...
                #print(f"state_i: {state_i}, type: {type(state_i)}")

                state.append(state_i)
                stamp.append(key_data[i]["t"])
                #length.append(len(state_i))
                # 如果 state_i 是一个可以调用 len 的对象（如 str、list、tuple、dict），则使用 len()
                if isinstance(state_i, (str, list, tuple, dict)):
                    length.append(len(state_i))
                else:
                    # 如果 state_i 是 float 类型，则可以跳过 len() 或者给 length 添加默认值
                    length.append(1)  # 如果是 float 或其他不可用 len() 的类型，添加默认长度值
...
def save_dict_to_hdf5(data: dict, target_path: str, pad_max_len: Optional[int] = None):
    ...
    with h5py.File(target_path, "w", rdcc_nbytes=1024**2 * 2) as root:
        for key, value in data.items():
            ...
            # 获取最大长度
            max_len = max(len(arr) for arr in value)

            # 填充每个数组，使其具有相同的长度
            value = [np.pad(arr, (0, max_len - len(arr)), mode='constant', constant_values=0) for arr in value]

            # 将填充后的数组组合成一个 numpy 数组
            value_array = np.array(value)

            # 然后将这个新的 value_array 传递给 HDF5 保存函数

            print(f"value: {value}")
            print(f"Value shape: {[len(v) for v in value]}")  # 输出每个元素的长度
            root.create_dataset(
                key, data=np.array(value_array, dtype=array_type), dtype=dtype, chunks=chunks
            )
