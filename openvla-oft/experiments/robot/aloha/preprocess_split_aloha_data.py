...
def load_hdf5(demo_path):
    ...
    with h5py.File(demo_path, "r") as root:
        ...
        for cam_name in root["/observations/images/"].keys():
            compressed_image_data = root[f"/observations/images/{cam_name}"][()]
            # Decompress the image
            compresser = crd.Compresser("jpg", [int(cv2.IMWRITE_JPEG_QUALITY), 50], True)
            decompressed_image = compresser.decompress(compressed_image_data, "jpg")
            
            image_dict[cam_name] = decompressed_image  # Store the decompressed image
    print(f"Loading episode complete: {demo_path}")

    return qpos,  action, image_dict # , is_sim qvel, effort
...
def load_and_preprocess_all_episodes(demo_paths, out_dataset_dir):
    ...
    for demo in tqdm(demo_paths):
        qpos, action, image_dict= load_hdf5(demo)#is_sim , qvel, effort, 
        # Save non-image info
...
def randomly_split(full_qpos, full_action, full_image_dict, percent_val):
    ...
    train_dict = dict(
        ...
        # ),
        image_dict=dict(
            cam_high=shuffled_image_dict["cam2"][:num_episodes_train],
            cam_left_wrist=shuffled_image_dict["cam1"][:num_episodes_train],
