...
def pkl_files_to_hdf5_and_video(pkl_files, hdf5_path, video_path):
    ...
    # hjy changes this
    images_to_video(np.array(data_list["observation"]["head_camera"]["rgb"]), out_path=f'{video_path}/head_cam.mp4')
    images_to_video(np.array(data_list["observation"]["front_camera"]["rgb"]), out_path=f'{video_path}/front_cam.mp4')
    images_to_video(np.array(data_list["observation"]["left_camera"]["rgb"]), out_path=f'{video_path}/left_cam.mp4')
    images_to_video(np.array(data_list["observation"]["right_camera"]["rgb"]), out_path=f'{video_path}/right_cam.mp4')  # 修正了拼写错误
